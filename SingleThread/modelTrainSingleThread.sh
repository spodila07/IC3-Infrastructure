#!/bin/bash
#SBATCH --job-name=modelTrainSingleThread_job_test     # Job name
#SBATCH --mail-type=ALL                # Mail events (NONE, BEGIN, END, FAIL, ALL)
#SBATCH --mail-user=<email_address>    # Where to send mail
#SBATCH --nodes=1                      # Use one node
#SBATCH --ntasks=1                     # Run a single task
#SBATCH --cpus-per-task=1              # Use 1 core` (change as needed)
#SBATCH --mem=2G                       # Memory limit (change as needed)
#SBATCH --time=00:05:00                # Time limit (hrs:min:sec)
#SBATCH --output=modelTrainSingleThread_test_%j.out    # Standard output and error log
...
python modelTrainSingleThread.py
